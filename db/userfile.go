...
// 更新用户文件表
func OnUserFileUploadFinshed(username, filehash, filename string, filesize int64) bool {
	stmt, err := mydb.DBConn().Prepare(
		"insert into tbl_user_file (user_name,file_sha1,file_name,file_size,last_update)values(?,?,?,?,?)",
	)
	if err != nil {
		return false
	}
